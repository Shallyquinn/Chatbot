...
class FPMChangeProvider implements FPMChangeProviderInterface {
  ...
  handleCurrentFPMSelection = (method: string): void => {
    ...
    this.setState((prev: ChatbotState) => ({
      ...
      currentFPMMethod: method, // Ensure currentFPMMethod is set
      currentStep: "fpmConcernType" as ChatStep,
    }));
    console.log("🔍 State updated, currentStep set to: fpmConcernType");
  };
  ...
  handleFPMConcernTypeSelection = (concernType: string): void => {
    ...
    // Store the current method
    this.currentConcernType = concernType;
    console.log("About to call getSpecificConcernResponse with:", {
      method: this.currentMethod,
      concernType: concernType,
      stateMethod: this.state.currentFPMMethod,
      methodType: typeof this.currentMethod,
      methodLength: this.currentMethod.length,
    }); // Debug log

    // Get specific response based on method and concern type
    const response = getSpecificConcernResponse(
      this.currentMethod,
      concernType
    );
