  ...
  parse(message: string): void {
    ...
    if (currentStep && ConfusionDetector.isStrictButtonStep(currentStep)) {
      ...
      if (confusionResult.isConfused && confusionResult.confidence > 0.6) {
        ...
        const helpMessage = ConfusionDetector.getHelpMessage(
          confusionResult,
          currentStep
        );
        ...
        const userSessionId =
          localStorage.getItem("userSessionId") || "default-session";
      ...
      default:
        ...
        if (lowerCase.includes("improve sex life")) {
          ...
          const genericResponse = this.actionProvider.createChatBotMessage(
            "I'm not sure I understand. If you need help with family planning methods, please select an option from the menu or type 'human' to speak with a human agent. You can:\n" +
