...
export const getAllLGAs = (): string[] => {
  return nigerianStates.flatMap((state) => state.lgas.map((lga) => lga.name));
};

export const getStateByLGA = (lgaName: string): State | undefined => {
  return nigerianStates.find((state) =>
    state.lgas.some((lga) => lga.name.toLowerCase() === lgaName.toLowerCase()),
  );
};

export const searchStates = (query: string): State[] => {
  const lowercaseQuery = query.toLowerCase();
  return nigerianStates.filter(
    (state) =>
...
export const searchLGAs = (query: string): { state: string; lga: string }[] => {
  const lowercaseQuery = query.toLowerCase();
  const results: { state: string; lga: string }[] = [];

  nigerianStates.forEach((state) => {
    state.lgas.forEach((lga) => {
      if (lga.name.toLowerCase().includes(lowercaseQuery)) {
        results.push({ state: state.name, lga: lga.name });
      }
    });
  });
