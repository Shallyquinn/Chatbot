...
import config from './yoruba-chatbot/config';
import MessageParser from './yoruba-chatbot/MessageParser';
import ActionProvider from './yoruba-chatbot/ActionProvider';
import ChatbotWithDividers from './components/ChatbotWithDividers';
import { ChatbotState, ChatMessage } from './yoruba-chatbot/types';
import Header from './components/Header';
import './App.css';

const YorubaChatbotApp: React.FC = () => {
  const navigate = useNavigate();
  const [isMobile, setIsMobile] = useState<boolean>(false);
  const chatKey = useMemo(() => 'yoruba-chatbot-session', []);
  ...
  const handleSaveMessages = useCallback((messages: ChatMessage[]) => {
    if (messages.length !== lastMessageCountRef.current) {
      ...
      try {
        localStorage.setItem('yoruba_chat_messages', JSON.stringify(messages));
        const currentState = JSON.parse(localStorage.getItem('yoruba_chat_state') || '{}');
        localStorage.setItem('yoruba_chat_state', JSON.stringify({ ...currentState, messages }));
      } catch (error) {
        console.error('Error saving Yoruba messages:', error);
      }
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const savedState = localStorage.getItem('yoruba_chat_state');
    if (savedState) {
      try {
        ...
        console.error('Failed to parse saved Yoruba state:', error);
      }
    }
  }, []);
...
export default YorubaChatbotApp;
