...
class FPMChangeProvider implements FPMChangeProviderInterface {
  ...
  handleCurrentFPMSelection = async (method: string): Promise<void> => {
    ...
    await this.api.createOrUpdateUser({ current_fpm_method: method }).catch(err => console.error('Failed to save FPM interaction:', err));
  ...
  handleFPMConcernTypeSelection = async (
    ...
    await this.api.createFpmInteraction({ fpm_flow_type: concernType, fpm_concern_type:concernType }).catch(err => console.error('Failed to save FPM interaction:', err));
    ...
    this.setState((prev: ChatbotState) => ({
      ...
      currentStep: 'fpmConcern' as ChatStep,
  ...
  handleFPMNextAction = async (action: string): Promise<void> => {
    ...
    await this.api.createResponse({
      ...
      available_options: ['End this chat,Ask more questions,Talk to AI / Human,Find nearest clinic'],
