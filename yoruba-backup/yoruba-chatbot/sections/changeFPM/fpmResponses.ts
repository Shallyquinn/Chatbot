...
export function getSpecificConcernResponse(
  ...
  if (!methodResponses[normalizedConcern]) {
    console.warn('⚠️ Concern not found for method:', {
      normalizedMethod,
      normalizedConcern,
    });
    console.log(
      'Available concerns for',
      normalizedMethod,
      ':',
  ...
  console.log('✅ Found specific response for:', {
    normalizedMethod,
    normalizedConcern,
  });
...
export function hasConcernResponse(method: string, concern: string): boolean {
  ...
  return (
    normalizedMethod in responses &&
    normalizedConcern in responses[normalizedMethod]
  );
}
...
export function getAvailableConcerns(method: string): string[] {
  const normalizedMethod = normalizeMethodKey(method);
  return responses[normalizedMethod]
    ? Object.keys(responses[normalizedMethod])
    : [];
}
